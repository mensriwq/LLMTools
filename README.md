...
- **策略建议 (`llm_next`)**: 根据当前的证明目标，动态建议最高效的下一步策略。
- **框架生成 (`llm_framework`)**: 为复杂的证明目标一键生成高级的证明框架（如 `induction`, `cases`），并用 `sorry` 填充待证明的子目标。
- **结束目标 (`llm_done`)**: 尝试结束目标。
- **引理补全 (`llm_type`)**: 为复杂的证明目标生成一个引理，并用 `sorry` 填充证明项。
- **代码修复 (`llm_revise`)**: 修复可能错误的证明，沿用原有的 `sorry`。
- **自动决策 (`llm_auto`,`llm_auto_one`)**: 自动进行问题分发, 分发给其他 `llm_` 策略。
...
本项目采用 Lean 4 作为前端，通过标准的输入/输出（Stdio）与一个独立的 Python 服务进行通信。
...
require «llm-tools» from git "https://github.com/mensriwq/LLMTools.git" @ "main"
```
...
建议在你的项目中为 AI 服务使用独立的 Python 虚拟环境。如果使用虚拟环境, 则需要配置环境变量 `LEAN_LLM_PYTHON`, 参考下一节。
...
AI 服务需要 API 密钥和其他配置才能运行。你需要创建以下环境变量, 请参考不同系统上设置环境变量的方法。你可以通过配置文件来覆盖环境变量, 请进入 `.lake/packages/llm-tools/` 目录, 除 `LEAN_LLM_PYTHON` 外, 其他变量均可新建文件 `LLMTools/LLMService/config.json` 来配置, 格式请参考 `LLMTools/LLMService/config.template.json`.
